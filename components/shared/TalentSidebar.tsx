...
import { Mic2, CalendarClock, Video, User, LogOut, Menu, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import Image from "next/image";

const sidebarLinks = [
  { href: "/talent/dashboard", label: "Dashboard", icon: Mic2 },
  { href: "/talent/schedule", label: "Jadwal Tampil", icon: CalendarClock },
  { href: "/talent/zoom", label: "Zoom Mentoring", icon: Video },
  { href: "/talent/profile", label: "Profil", icon: User },
];
    ...
    <>
      ...
      <aside className={`${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky top-0 lg:top-0 left-0 z-40 w-64 h-screen bg-card border-r border-border transition-transform duration-300 lg:flex lg:flex-col pt-16 lg:pt-0`}>
        <div className="hidden lg:flex flex-col gap-2 p-6 border-b border-border">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-purple-600 bg-purple-500/10 px-2 py-0.5 rounded-full w-fit">
            Talent
          </span>
        </div>
